...
def getmacdrsi(ticker):
    ...
    macd = MACD(df_macd['trade_price'])[0]
...
def get_ma(ticker, day):
    df = pyupbit.get_ohlcv(ticker, interval="minute", count=80)
    ma = df['close'].rolling(window=day, min_periods=1).mean().iloc[-1]
    return ma
